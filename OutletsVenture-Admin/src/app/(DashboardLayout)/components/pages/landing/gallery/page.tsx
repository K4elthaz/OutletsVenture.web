...
import { landingPageGallery, landingPageGalleryRef } from "@/utils/References";
...
const Page = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, landingPageGallery);
  ...
  const handleSave = async (values: any) => {
    ...
    if (fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        landingPageGalleryRef(photoID, file.name)
      );
    ...
    if (editCard) {
      await update(dbRef(db, `${landingPageGallery}/${picture.id}`), picture);
    } else {
      await set(dbRef(db, `${landingPageGallery}/${picture.id}`), picture);
    }
    ...
    getMetadata(storageReference)
      .then(() => {
        deleteObject(storageReference)
          .then(() => {
            remove(dbRef(db, `${landingPageGallery}/${id}`))
              .then(() => {
                setPhotos((photos) =>
  ...
  return (
    <PageContainer title="Landing Page">
      {/* Breadcrumbs */}
