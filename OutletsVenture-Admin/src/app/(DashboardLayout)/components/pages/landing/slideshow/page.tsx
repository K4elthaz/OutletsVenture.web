...
const Page = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, landingPageSlideShow);
  ...
  const handleSave = async (values: any) => {
    ...
    if (fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        landingPageSlideShowRef(photoID, file.name)
      );
    ...
    if (editCard) {
      await update(dbRef(db, `${landingPageSlideShow}/${picture.id}`), picture);
    } else {
      await set(dbRef(db, `${landingPageSlideShow}/${picture.id}`), picture);
    }
    ...
    getMetadata(storageReference)
      .then(() => {
        deleteObject(storageReference)
          .then(() => {
            remove(dbRef(db, `${landingPageSlideShow}/${id}`))
              .then(() => {
                setPhotos((photos) =>
  ...
  return (
    <PageContainer title="Landing Page" description="This is the Landing Page">
      ...
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 3 }}>
        ...
        <Typography color="text.primary">Slide Show</Typography>
      </Breadcrumbs>
