...
const Page = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, landingPageCarousel2);
  ...
  const handleSave = async (values: any) => {
    ...
    if (fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        landingPageCarousel2PhotoRef(photoID, file.name)
      );
    ...
    if (editCard) {
      await update(dbRef(db, `${landingPageCarousel2}/${picture.id}`), picture);
    } else {
      await set(dbRef(db, `${landingPageCarousel2}/${picture.id}`), picture);
    }
    ...
    getMetadata(storageReference)
      .then(() => {
        deleteObject(storageReference)
          .then(() => {
            remove(dbRef(db, `${landingPageCarousel2}/${id}`))
              .then(() => {
                setPhotos((photos) =>
  ...
  return (
    <PageContainer title="Landing Page" description="This is the Landing Page">
      ...
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 3 }}>
        ...
        <MuiLink href="/pages/home/landing" color="inherit">
          Landing Page
        </MuiLink>
        <Typography color="text.primary">Carousel 2</Typography>
      </Breadcrumbs>
