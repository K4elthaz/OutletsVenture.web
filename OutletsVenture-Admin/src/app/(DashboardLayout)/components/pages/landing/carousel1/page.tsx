...
import {
  landingPageCarousel1,
  landingPageCarousel1PhotoRef,
...
const Page = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, landingPageCarousel1);
  ...
  const handleSave = async (values: any) => {
    ...
    if (fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        landingPageCarousel1PhotoRef(photoID, file.name)
      );
    ...
    if (editCard) {
      await update(dbRef(db, `${landingPageCarousel1}/${picture.id}`), picture);
    } else {
      await set(dbRef(db, `${landingPageCarousel1}/${picture.id}`), picture);
    }
    ...
    getMetadata(storageReference)
      .then(() => {
        deleteObject(storageReference)
          .then(() => {
            remove(dbRef(db, `${landingPageCarousel1}/${id}`))
              .then(() => {
                setPhotos((photos) =>
  ...
  return (
    <PageContainer title="Landing Page" description="This is the Landing Page">
      ...
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 3 }}>
        ...
        <MuiLink href="/components/pages/landing" color="inherit">
          Landing Page
        </MuiLink>
        <Typography color="text.primary">Carousel 1</Typography>
      </Breadcrumbs>
        ...
        <div>
          {
            ...
            <div>
              <Stack
                ...
                sx={{ mb: 3 }}
              >
                <Link href="/components/pages/landing" passHref>
