...
const Page = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, homePageCarousel1);
  ...
  const handleSave = async (values: any) => {
    ...
    if (fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        homePageCarousel1PhotoRef(photoID, file.name)
      );
    ...
    if (editCard) {
      await update(dbRef(db, `${homePageCarousel1}/${picture.id}`), picture);
    } else {
      await set(dbRef(db, `${homePageCarousel1}/${picture.id}`), picture);
    }
    ...
    getMetadata(storageReference)
      .then(() => {
        deleteObject(storageReference)
          .then(() => {
            remove(dbRef(db, `${homePageCarousel1}/${id}`))
              .then(() => {
                setPhotos((photos) =>
