...
const Page = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, homePageSlideShow);
  ...
  const handleSave = async (values: any) => {
    ...
    if (fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        homePageSlideShowPhotoRef(photoID, file.name)
      );
    ...
    if (editCard) {
      await update(dbRef(db, `${homePageSlideShow}/${picture.id}`), picture);
    } else {
      await set(dbRef(db, `${homePageSlideShow}/${picture.id}`), picture);
    }
    ...
    getMetadata(storageReference)
      .then(() => {
        deleteObject(storageReference)
          .then(() => {
            remove(dbRef(db, `${homePageSlideShow}/${id}`))
              .then(() => {
                setPhotos((photos) =>
