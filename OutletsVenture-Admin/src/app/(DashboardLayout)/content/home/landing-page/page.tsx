...
const LandingPage = () => {
  const [landingPageCards, setLandingPageCards] = useState<Data[]>([]);
  ...
  useEffect(() => {
    const starCountRef = dbRef(db, "Home/Landing Page");
  ...
  const showModal = (card?: any, isNested: boolean = false) => {
    ...
    setSelectedNestedCard(isNested ? card : null); // Set selectedNestedCard if it's a nested card
  };
  ...
  const handleSave = async (values: any) => {
    ...
    const promoID = editCard
      ? editCard.id
      : landingPageCards.length === 0
      ? 1
      : landingPageCards[landingPageCards.length - 1].id + 1;
    ...
    if (fileList && fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        `Home/Landing Page/${promoID}/${file.name}`
      );
    ...
    if (editCard) {
      await update(dbRef(db, "Home/Landing Page/" + item.id), item);
    } else {
      await set(dbRef(db, "Home/Landing Page/" + item.id), item);
    }
  ...
  const handleNestedSave = async (values: any) => {
    ...
    if (fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        `Home/Landing Page/${selectedParentCard.id}/NestedCards/${newNestedCardId}/${file.name}`
      );
    ...
    await update(
      dbRef(db, `Home/Landing Page/${selectedParentCard.id}`),
      updatedParentCard
    );
  ...
  const handleEditNestedSave = async (values: any) => {
    ...
    if (fileList.length > 0) {
      ...
      const storageReference = storageRef(
        getStorage(),
        `Home/Landing Page/${selectedParentCard.id}/NestedCards/${selectedNestedCard.id}/${file.name}`
      );
    ...
    getMetadata(storageReference)
      .then(() => {
        deleteObject(storageReference)
          .then(() => {
            remove(dbRef(db, `Home/Landing Page/${id}`))
              .then(() => {
                setLandingPageCards((prevCards) =>
  ...
  const handleDeleteNestedCard = async (nestedId: number) => {
    ...
    const storageReference = storageRef(
      getStorage(),
      `Home/Landing Page/${selectedParentCard.id}/NestedCards/${nestedId}`
    );
  ...
  const handleBack = () => {
    setSelectedParentCard(null); // Return to the landing page
  };
